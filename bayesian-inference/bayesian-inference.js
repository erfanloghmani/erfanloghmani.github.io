...
function prior() {
	...
	//Determines outcome of coin flip and updates data
	function flipSelected(coin, selected) {
		var img = selected ? "url(./img/head.png)" : "url(./img/tail.png)";
		n += 1;
		count += (selected ? 1 : 0);
		coin.css("background-image", img);
		data.push(posterior(alpha + count, beta + n - count));
	...
	$('#reset_prior').click(function() {
		reset();
	});
	// flip heads
	$('#flip_heads').click(function() {
		var coin = $("#coin");
